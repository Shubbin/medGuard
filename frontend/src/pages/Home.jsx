...
const Home = () => {
  ...
  return (
    <main className="relative min-h-screen bg-white px-6 py-16 sm:px-12 md:px-20 overflow-hidden">
      <Helmet>
        <title>MedGuard — Verify Drugs Instantly</title>
        <meta name="description" content="MedGuard uses AI to verify drug authenticity and fight fake medications in seconds." />
        <meta property="og:title" content="MedGuard — AI-Powered Drug Safety" />
      ...
      <div className="relative z-10 max-w-6xl mx-auto text-center">
        {/* Hero Section */}
        <section className="relative rounded-2xl overflow-hidden shadow-lg mb-16" role="region" aria-labelledby="hero-title">
          ...
          <div className="absolute inset-0 flex flex-col items-center justify-center px-4">
            <motion.h1
              ...
              className="text-5xl md:text-6xl font-extrabold bg-gradient-to-r from-primary to-primary-dark bg-clip-text text-transparent"
            >
        ...
        <motion.div
          ...
          className="mt-4 inline-block px-4 py-2 bg-green-100 text-green-800 text-sm rounded-full"
        >
        ...
        >
          <motion.div variants={fadeUp}>
            <Link to={isAuthenticated ? "/verify" : "/login"} className="bg-primary text-blue-500 px-6 py-3 rounded-xl flex items-center gap-2 shadow hover:scale-105 transition">
              <ScanSearch className="w-5 h-5" /> {isAuthenticated ? "Verify Now" : "Login to Start"}
            </Link>
          </motion.div>
          <motion.div variants={fadeUp}>
            <Link to={isAuthenticated ? "/dashboard" : "/signup"} className="border border-primary text-primary px-6 py-3 rounded-xl flex items-center gap-2 hover:bg-primary/5 transition">
              <ShieldCheck className="w-5 h-5" /> {isAuthenticated ? "Report Drug" : "Create Account"}
            </Link>
          </motion.div>
        </motion.div>

        {/* Features */}
        <section aria-labelledby="features-heading" className="mt-20" role="region">
          ...
          >
            {features.map((feature, index) => (
              <motion.div
                key={index}
                variants={fadeUp}
                className="bg-white border border-gray-100 rounded-xl shadow hover:shadow-xl transition-all overflow-hidden"
              >
                <img src={feature.image} alt={feature.title} className="h-40 w-full object-cover transition-transform duration-500 hover:scale-105" loading="lazy" />
                <div className="p-6">
        ...
        {/* Stats */}
        <section aria-labelledby="stats-heading" className="mt-20" role="region">
          ...
          >
            {stats.map((stat, index) => (
              <motion.div key={index} variants={fadeUp} className="bg-primary/5 p-6 rounded-xl shadow">
        ...
        {/* Testimonials */}
        <section aria-labelledby="testimonials-heading" className="mt-24" role="region">
          <h2 id="testimonials-heading" className="text-3xl font-bold text-primary-dark mb-10">Real Stories. Real Impact.</h2>
          <motion.div
          ...
          >
            {testimonials.map((t, i) => (
              <motion.div key={i} whileHover={{ scale: 1.03 }} className="bg-white p-6 rounded-xl shadow border border-gray-100 text-left transition">
        ...
        <section className="mt-24" aria-labelledby="video-heading" role="region">
          <h2 id="video-heading" className="text-2xl font-bold text-gray-800 mb-6">How MedGuard Works</h2>
